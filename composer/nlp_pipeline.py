...
def nlp_pipeline():
    ...
    # export_notes_to_storage = BigQueryInsertJobOperator(
    #     task_id='export_notes_to_storage',
    #     configuration={
    #         'query': {
    #             'query':
    #             f"SELECT * FROM {PROJECT_ID}.{NOTES_BQ_DATASET}.{HPO_ID}_note LIMIT 100 ",
    #             'useLegacySql': False,
    #             'timeoutMs': 5000
    #         }
    #     },
    ...
    export_notes_to_storage = BigQueryGetDatasetTablesOperator(
        task_id="export_notes_to_storage",
        dataset_id='composer_test',
    ...
    #     logging.info('Getting tables list')
    #     tables = hook.get_dataset_tables_list(
    #         'composer_dataset_name', project_id='{PROJECT_ID}')
    #     logging.info('Finish getting tables list')
    ...
    #     # export_query = f"SELECT * FROM {PROJECT_ID}.{NOTES_BQ_DATASET}.{HPO_ID}_note LIMIT 100 "
    #     export_query = f"SELECT 1 "
    ...
    # bash_check >> export_notes_to_storage >> run_clamp >> import_note_nlp_to_bigquery(
    # )

    bash_check >> export_notes_to_storage


nlp_pipeline_dag = nlp_pipeline()

if __name__ == "__main__":
    nlp_pipeline_dag.test()
